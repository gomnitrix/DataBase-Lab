...
class BpTreeInterNode(object):
    def __init__(self, m):
        if not isinstance(m, int):
            raise InitError('M must be int')
        if m <= 3:
            raise InitError('M must be greater than 3')
        else:
            self.__M = m
            self.clist = []
            self.ilist = []
...
class BpTree(object):
    ...
    def insert(self, key_value):
        ...
        def split_node(n1):
            mid = self.M // 2
            newnode = BpTreeInterNode(self.M)
            ...
            for c in newnode.clist:
                c.par = newnode
            if n1.par is None:
                newroot = BpTreeInterNode(self.M)
            ...
            if mi == ma:
                ...
                if l1 is l2:
                    if i1 == i2:
                        ...
                        result.extend(l1.vlist[i1:i2])  # TODO maybe wrong
                        return result
                else:
